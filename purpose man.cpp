...
void women_open(int speed,int color)
{
	int s=speed;
...
// man rose break
void man_rose_break(int speed,int color)
{
		int s=speed;
...
void women_close(int speed,int color)
{
	int s=speed;
...
void women_chat(int speed,int box_color,int text_color)
{
	int s=speed;
	setcolor(box_color);
   ...
   setcolor(text_color);
    settextstyle(EUROPEAN_FONT,HORIZ_DIR,1);
	outtextxy(45+s,210,"Sorry, I have a");
	outtextxy(100+s,240,"Boyfriend");
...
int main()
{
	...
	// women walk 70
		...
		if(n>70&&n<=170)
		{
			...
			man_rose(sm,14);
			if(n%2==0)
			women_open(280+sw,15);
			else
			women_close(280+sw,15);
			sw+=4;
		}
    //cloud move 50
